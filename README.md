...
- **Console de Administração (equipe Gerti):** em [`apps/admin/`](apps/admin/) — Nuxt 3 SSR de identidade FIXA Gerti (não white-label); login de **agente Znuny** (cookie próprio `gsid_adm`), **onboarding de cliente** num fluxo (tenant + branding + usuários/papéis, escrita no Znuny via webservice GI custom `GertiAdmin`) e **criar contrato** dos 6 tipos (#1G-a, ADR D19). Cross-tenant via caminho admin BYPASSRLS, isolado do portal do cliente. Subdomínio próprio (`gerti.was.dev.br` em teste; `admin.suporte.gerti.com.br` em prod)
