...
# Wraps the native Kernel::System::CustomerCompany->CustomerCompanyAdd so the
# Gerti admin sidecar can create a CustomerCompany via GI (Spec #0: writes to
# Znuny are ALWAYS via GI, never direct SQL). Shipped through the upgrade-safe
# Custom/ overlay (same mechanism as Custom/Kernel/System/Cache/Redis.pm).
# --
...
Kernel::GenericInterface::Operation::CustomerCompany::CustomerCompanyAdd
- GenericInterface CustomerCompany Add operation backend (Gerti custom).
...
sub Run {
    ...
    if ( !IsHashRefWithData( $Param{Data} ) ) {
        return $Self->ReturnError(
            ErrorCode    => 'CustomerCompanyAdd.MissingParameter',
            ErrorMessage => 'CustomerCompanyAdd: the request is empty!',
        );
    }
    ...
    for my $Needed (qw(CustomerID CustomerCompanyName)) {
        if ( !IsStringWithData( $Param{Data}->{$Needed} ) ) {
            return $Self->ReturnError(
                ErrorCode    => 'CustomerCompanyAdd.MissingParameter',
                ErrorMessage => "CustomerCompanyAdd: $Needed parameter is missing!",
            );
        }
    }
    ...
    if ( !$CustomerID ) {
        return $Self->ReturnError(
            ErrorCode    => 'CustomerCompanyAdd.AddError',
            ErrorMessage => 'CustomerCompanyAdd: could not create the customer company'
                . ' (maybe it already exists or the input was rejected).',
        );
    }
...
(C<GertiAdmin::AccessToken> in Znuny config). Fails closed: a missing
configured token, a missing request token, or a mismatch all reject.
