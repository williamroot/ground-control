...
| Onda 1 (R1): migration `0027_tenant_address_contact` — endereço e contato em `gerti.tenant` (10 colunas nullable). `gerti.tenant` é a DONA (decisão D-B); o Znuny recebe espelho best-effort | **Pronto + DEPLOYADO em staging** (verificado ao vivo) |
| Onda 1 (R1): `PUT /v1/admin/tenants/{id}` + GI op `CustomerCompanyUpdate` (merge sobre o atual). `subdomain` e `znuny_customer_id` imutáveis → 422 | **Pronto + DEPLOYADO em staging** |
| Onda 1 (R2): migration `0028_portal_user_contact_flags` — `email_intake_enabled` (default `true`) e `extension` em `portal_user_role`. Ramal fica do nosso lado porque o `customer_user` nativo não tem o campo e a invariante 4 proíbe coluna nova no núcleo | **Pronto + DEPLOYADO em staging** |
| Onda 1 (R2): GI ops `CustomerUserUpdate` (editar/desativar, `ValidID=2`, nunca delete) e `CustomerUserList` (pessoas por `CustomerID`, válidas e inválidas) + `GET/POST/PUT /v1/admin/tenants/{id}/users*`. Znuny é o dono da identidade (D-C); a resposta junta identidade+contato do Znuny com papel+flags nossos | **Pronto + DEPLOYADO em staging** (usuário criado direto no Znuny aparece no console) |
| Onda 1 (R2/T-R2.3): migration `0030_consumption_orphan` + auto-vínculo no `reconciliation_service`. Hora em chamado SEM contrato (o caso do e-mail) era descartada em silêncio; agora vincula ao único contrato ativo, ou grava pendência reprocessável (`no_tenant`/`no_active_contract`/`ambiguous_contract`). Cursor segue avançando por decisão D-E. `TimeAccountingSince` ganhou LEFT JOIN em `ticket` para trazer `CustomerId` | **Pronto + DEPLOYADO em staging** |
| Onda 1 (R5): migration `0029_tenant_queue` (`gerti.tenant_queue`, FORCE RLS, índice parcial único = no máx. 1 fila padrão por cliente) + `GET/PUT /v1/admin/tenants/{id}/queues`. Valida cada fila contra a lista viva do Znuny; fila inexistente recusa o conjunto inteiro (422) | **Pronto + DEPLOYADO em staging** |
| Onda 1 (R5): fila padrão do cliente aplicada na abertura de chamado — `'Raw'` deixa de ser destino silencioso. Fila informada precisa estar associada, senão 422 | **Pronto + DEPLOYADO em staging** |
| Onda 1 (R5): `AdminGroupList` devolve `RwUserCount` (agentes com `rw` no grupo) → coluna "atendida por" na tela de filas | **Pronto + DEPLOYADO em staging** |
| Onda 3 (R18a): `ContractReadService.series` ganha janela (`window=cycles\|months`, `count`) além do comportamento histórico (vida inteira, dia a dia) que o portal usa e que **não mudou**. Chaves `CONSUMPTION_WINDOW_MODE`/`CONSUMPTION_WINDOW_COUNT`, sobrescritíveis por requisição | **Pronto + DEPLOYADO em staging** |
| Onda 3 (R18a): `GET /v1/admin/tenants/{id}/consumption-series` — uma série por contrato ativo, cada uma na unidade do seu contrato; contrato sem saldo não vira gráfico vazio | **Pronto + DEPLOYADO em staging** |
| Onda 3 (R18b): `TicketStats` agrega por tipo/serviço/fila e devolve a lista de chamados com horas (`IncludeTickets`, opt-in — o painel de analytics usa a mesma op e não paga a conta) | **Pronto + DEPLOYADO em staging** |
| Onda 3 (R18b): `ReportService` + `report_pdf` + `templates/executive_report.html` — consumo na unidade do contrato, principais chamados e a listona final, em PDF branded. Fallback ReportLab **pagina** (ao contrário do da fatura) | **Pronto + DEPLOYADO em staging** |
...
| #1N: IA via **Ollama Cloud `gpt-oss:120b`** (`integrations/ollama.py`, `/api/chat` NDJSON, failure-safe) — `POST /v1/admin/ai/summarize` + `/suggest-reply` (só agente, opt-in `AI_FEATURES_ENABLED`), `AgentTicketGet` já trazia a thread; `ai_generation_log` (só metadados, sem conteúdo). **Defesa contra prompt injection** (spotlighting/delimitadores, sanitização, sem tools, saída não-confiável, teste de regressão). Painel `AiPanel` no console (rascunho editável, nunca auto-envia) | **Pronto + DEPLOYADO em staging (2026-06-09, e2e ao vivo)** |
| #1O: Dashboards por tenant (híbrido) — GI **`TicketStats`** (contagens por `CustomerID`, anti-IDOR, ByState/ByPriority/ByDay/SLA), `MetricsService` agrega CSAT(#1M)+horas+saldo(#1B)+tickets (failure-soft), `GET /v1/dashboard/metrics` (portal admin) + `GET /v1/admin/analytics?tenant_id=` (console, "Todos" agrega client-side), charts SVG `BarChart`/`DonutChart` (brand/semântico, H8). OpenSearch Dashboards só ad-hoc interno | **Pronto + DEPLOYADO em staging (2026-06-09, e2e ao vivo)** |
| #1P: Faturas internas (não-fiscais) — `invoice`+`invoice_line` (RLS, UNIQUE `(tenant,number)`), `InvoiceService.create_from_cycle` (1/ciclo idempotente, numeração sob advisory lock), PDF branded **WeasyPrint**+Jinja2 (bytea, on-demand), worker marca `overdue` 1x/dia; `GET /v1/invoices`+`/pdf` (portal admin), `POST /v1/admin/tenants/{id}/invoices`+`/paid`+`/void` (console); status semânticos (open/paid/overdue/void, H8) | **Pronto + DEPLOYADO em staging (2026-06-09, e2e ao vivo)** |
| #1Q: Motor de automação próprio — Event module Znuny **`GertiAutomation`** (assina HMAC via LWP, posta eventos ao sidecar) → `POST /v1/hooks/znuny/ticket-event` (verifica HMAC, resolve tenant por `CustomerID`) → `AutomationEngine` (DSL pura allowlist, sem `eval`) → ações via GI **`AgentTicketUpdate`** (set queue/state/priority + nota; `ai_summarize_note` reusa #1N). `automation_rule`/`automation_run` (RLS), CRUD + UI no-code no console. Anti-loop (só `ArticleCreate` de cliente) | **Pronto + DEPLOYADO em staging (2026-06-09, e2e ao vivo)** |
| #1R-a: Auto-registro de equipamentos (servidor) — GI **`ConfigItemUpsert`** (escrita no CMDB, anti-IDOR, create/update por `ConfigItemID`, campo `Fingerprint`); `agent_enroll_token`/`device_agent` (RLS); `POST /v1/agent/enroll` (Bearer token→tenant **server-trusted**) + `/heartbeat` (Bearer secret); credenciais só sha256 (constant-time); híbrido auto+`pending` (dedupe por fingerprint, `max_registrations`); console por tenant (gerar/rotacionar token + comando de instalação, listar/aprovar/revogar dispositivos) | **Pronto + DEPLOYADO em staging (2026-06-10, e2e ao vivo)** |
| #1R-b: Agente **Go** (binário estático cross-platform `apps/agent/`) — subcomandos `enroll` (coleta fingerprint estável + specs, troca o enroll token por credencial própria, descarta o token) e `run` (loop de heartbeat com backoff, para em revogado); `packaging/install.sh` + serviço systemd; distribuição via sidecar `GET /v1/agent/install.sh?server=` + `GET /v1/agent/download/{os-arch}`. Binários buildados (`apps/agent/build.sh`) em `apps/sidecar/agent-dist/` antes do build do sidecar | **Pronto + VALIDADO em staging (2026-06-10, agente real registrou e bateu heartbeat via Cloudflare)** |
| #1S: Assistente de escrita de chamado por IA (portal) — `POST /v1/ticketing/assist` (cliente `gsid`, opt-in `AI_FEATURES_ENABLED`, rate-limit 20/h, anti-injeção) reusa `integrations/ollama.py` (#1N) → estrutura o texto do cliente (problema/início/impacto/ações) + título; botão "✨ Melhorar com IA" no `/tickets/novo` (rascunho editável, nunca auto-envia); `ai_assist_enabled` no form-meta; auditoria em `ai_generation_log` (kind=`assist`) | **Pronto + DEPLOYADO em staging (2026-06-10, e2e ao vivo)** |
| Onda 5 da campanha — **financeiro e fluxo**: `service_count` passa a consumir por CHAMADO (`consumption_event.znuny_ticket_id`) e a fatura ganha linhas em atendimentos; bolsa de crédito **compartilhada** de verdade (`credit_shared` ligado a `shared_credit_pool` lê o saldo do GRUPO); saldo acumulado com **teto e validade** por contrato (baldes datados em `cycle.totals.carry_buckets`, FIFO); mensalidade proporcional ao tamanho do ciclo (`billing_amount_period`); lançamentos avulsos (`ExtraChargeService`, como `consumption_event` — herda glosa/ciclo/fatura); contrato `free` para cliente sem contrato formal; **boleto e NFS-e pelo Asaas** a partir de `gerti.invoice` (`InvoiceChargeService`, idempotentes, baixa pelo webhook); **aprovação de chamado** (`ticket_approval`, estado Znuny `aguardando aprovacao` do tipo `pending reminder`, papel `approver`) | **Pronto + DEPLOYADO em staging (2026-08-19, e2e ao vivo)** |
